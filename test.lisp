...
(defmethod convert ((to-type (eql 'node-cons)) (sequence cons)
                    &key &allow-other-keys)
  (flet ((next (it)
           (etypecase it
...
(deftest @.error ()
  ...
  (is (handler-case (progn (@ (convert 'node-cons '(:a)) '(-1)) nil)
        (error () t)))
  (is (handler-case (progn (@ (convert 'node-cons '(:a (:b))) '(1)) nil)
        (error () t))))
