...
(deftest insert-after-named-children ()
  (let* ((l1 '((a b) (c d)))
         (n1 (convert 'node-list2 l1))
         (l2 '(f g))
         (n2 (convert 'node-list2 l2))
         (n3 (insert-after n1 '((body . 1)) n2)))
    (is (equal (convert 'list n3) '((a b) (c d) (f g))))))
...
(deftest splice-after-test ()
  (flet ((to-node (it)
           (make-instance 'node-with-data :data it)))
    (let ((it (convert 'node-with-data '(0 1 2 3 4))))
      (is (equalp (convert 'list (splice-after it '(1)
                                         (mapcar #'to-node '(:a :b :c))))
                  '(0 1 2 :a :b :c 3 4))))
    (let ((it (convert 'node-with-data '(0 1 2 3 4)))
          (n (mapcar #'to-node '(:a 5))))
      (is (equal (convert 'list (splice-after it '(1) n))
                 `(0 1 2 :a 5 3 4))))
    (let ((it (convert 'node-with-data '(:a (:b 1) (:c (:d 2) (:e 3) 4) 5))))
      (is (equal (convert 'list (splice-after it '(1 0) (mapcar #'to-node '(:new))))
                 '(:a (:b 1) (:c (:d 2) :new (:e 3)  4) 5)))
      (is (equal (convert 'list (splice-after it (@ it '(1 1))
                                        (mapcar #'to-node '(:new))))
                 '(:a (:b 1) (:c (:d 2) (:e 3) :new 4) 5))))
    (let ((it (convert 'node-with-fields '(:data :x :a (1) :b (2)))))
      (is (handler-case (progn (splice-after it '(:a) (to-node :what)) nil)
            (error () t))))))
...
(deftest insert-after-test ()
  (flet ((to-node (it)
           (make-instance 'node-with-data :data it)))
    (let ((it (convert 'node-with-data '(0 1 2 3 4))))
      (is (equalp (convert 'list (insert-after it '(1) (to-node :a)))
                  '(0 1 2 :a 3 4))))
    (let* ((it (convert 'node-with-data '(:a (:b 1) (:c 2 3 (:d 4) 5) 6)))
           (n (@ it '(1 2))))
      (is (equal (convert 'list (insert-after it '(1 2) (to-node :new)))
                 '(:a (:b 1) (:c 2 3 (:d 4) :new 5) 6)))
      (is (equal (convert 'list (insert-after it n (to-node :new)))
                 '(:a (:b 1) (:c 2 3 (:d 4) :new 5) 6))))))
...
(deftest error-tests ()
  (let* ((it (convert 'node-with-data '(:a (:b 1) (:c 2 3 (:d 4) 5) 6)))
    ...
    (is (handler-case (progn (splice it (convert 'node-with-data '(:e 10))
                             ...
                             nil)
          (error () t)))
    (is (handler-case (progn (splice-after it (convert 'node-with-data '(:e 10))
                                           (list n))
                             nil)
          (error () t)))
    (is (handler-case (progn (insert it (convert 'node-with-data '(:e 10)) n)
                             nil)
          (error () t)))
    (is (handler-case (progn (insert-after it (convert 'node-with-data '(:e 10)) n)
                             nil)
          (error () t)))))
...
(deftest test-walk-child-slot-children ()
  ...
  ;;                        (collect c)))
  ;;                    '(node-with-fields node-with-fields))))
  ;; (is (equal
  ;;      '(1 2)
  ;;      (serapeum:with-collectors (collect)
